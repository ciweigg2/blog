cover: http://ciwei2.cn-sh2.ufileos.com/98.jpg
title: Sentinel实战：集群限流环境搭建
date: 2019-01-30 14:59:32
tags: [sentinel,nacos]
categories: [综合]
---
## Sentinel实战：集群限流环境搭建

**本文demo在：https://github.com/ciweigg2/sentinel-tutorial/tree/master/sentinel-practice/sentinel-cluster-flow-control 章节**

下面我们来看一下如何快速使用集群流控功能。接入集群流控模块的步骤如下：

<!--more-->

原文：[sentinel集群限流](https://mp.weixin.qq.com/s?__biz=MzA3MTQ2MDgyOQ==&mid=2247484101&idx=1&sn=07cff3892c884dd9075e73dd2ae87dd5&chksm=9f2c7077a85bf961842d53dec73120bae2eae8b028b0c8f61f32e83fe720313a7bb51f657037&mpshare=1&scene=1&srcid=0127v0nz4yHamnPW6hrIGWkz#rd)

> 最简单的测试方式是：

* 下载demo

* 下载sentinel [release 页面](https://github.com/alibaba/Sentinel/releases) [sentinel文档](https://github.com/alibaba/Sentinel/wiki/%E6%8E%A7%E5%88%B6%E5%8F%B0)
...
* nacos搭建参考：[nacos搭建](https://ciweigg2.github.io/2019/01/29/an-zhuang-grafana-jian-kong-nacos/)
...
* http://localhost:8721/setClusterMode?mode=0 http://localhost:8722/setClusterMode?mode=0

### 启动配置中心

要想使用集群流控功能，我们需要在应用端配置动态规则源，并通过 Sentinel 控制台实时进行推送。如下图所示：

![](/images/push-cluster-rule.png)
...
![](/images/start-up-nacos.png)

如上图所示，启动成功后，我们可以访问 Nacos 的控制台来进行控制了：

![](/images/login-nacos-console.png)
...
![](/images/register-cluster-flow-rule.png)
...
![](/images/start-up-cluster-server-1.png)

在 ~/logs/csp/sentinel-record.log 日志文件中将打印如下信息：

![](/images/start-up-cluster-server-2.png)
...
![](/images/set-cluster-mode-client.png)
...
![](/images/call-cluster-client-1.png)
...
![](/images/client1-metrics.png)

![](/images/client2-metrics.png)
...
![](/images/fixed-real-time-monitor.png)

再看 metrics 日志文件，发现文件名也带上了进程号：

![](/images/fixed-metrics-logs.png)

再看下每个文件中的实际统计结果：

![](/images/fixed-client1-metrics.png)

![](/images/fixed-client2-metrics.png)

再看 MetricFetcher 中打印的日志，发现也没有再去请求 token-server 的 metric 了，如下图所示：

![](/images/fixed-metrics-fetcher.png)
...
![](/images/cluster-flow-summary.png)
